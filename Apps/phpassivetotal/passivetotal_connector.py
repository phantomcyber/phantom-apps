...
class PassivetotalConnector(BaseConnector):
    ...
    def _make_rest_call(self, endpoint, request_params, action_result):
        ...
        try:
            ...
            return (action_result.set_status(status_strings.APP_ERROR, PASSIVETOTAL_ERR_SERVER_CONNECTION, e), resp_json, status_code)
        ...
        try:
            ...
            action_result.set_status(status_strings.APP_ERROR, PASSIVETOTAL_ERR_FROM_SERVER, status=r.status_code, message=details)
            return (status_strings.APP_ERROR, resp_json, status_code)

        # Check if it's a success
        if (200 <= status_code <= 299):
            # Success
            return (status_strings.APP_SUCCESS, resp_json, status_code)
    ...
    def _test_connectivity(self, param):
        ...
        return self.set_status_save_progress(status_strings.APP_SUCCESS, PASSIVETOTAL_SUCC_CONNECTIVITY_TEST)
    ...
    def _lookup_domain(self, param):
        ...
        if start_time:
            try:
                ...
                return action_result.set_status(status_strings.APP_ERROR, 'Incorrect date format for start time, it should be YYYY-MM-DD')

        if end_time:
            try:
                datetime.strptime(end_time, '%Y-%m-%d')
            except ValueError:
                return action_result.set_status(status_strings.APP_ERROR, 'Incorrect date format for end time, it should be YYYY-MM-DD')
        ...
        if (not extra_data) and (status_strings.is_fail(ret_val)):
    ...
    def _whois_ip(self, param):
        ...
        if(not response):
            ...
            return action_result.set_status(status_strings.APP_SUCCESS, "No registrant info found")
