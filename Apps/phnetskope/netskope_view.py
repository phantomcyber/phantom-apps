    ...
    for pages in data.get('page', []):
        try:
            if pages.get("_insertion_epoch_timestamp"):
                pages['_insertion_epoch_timestamp'] = '{}Z'.format(datetime.fromtimestamp(
                    pages['_insertion_epoch_timestamp']).isoformat())
    ...
    for app in data.get('application', []):
        try:
            if app.get("_insertion_epoch_timestamp"):
                app['_insertion_epoch_timestamp'] = '{}Z'.format(datetime.fromtimestamp(
                    app['_insertion_epoch_timestamp']).isoformat())
        except ValueError:
            pass
