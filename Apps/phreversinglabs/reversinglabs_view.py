...
def file_reputation(provides, all_results, context):
    ...
    for summary, action_results in all_results:
        ...
        tally['total_found'] += int(summary.get(json_keys.APP_JSON_TOTAL_OBJECTS_SUCCESS, 0))
        tally['total_queried'] += int(summary.get(json_keys.APP_JSON_TOTAL_OBJECTS_TO_ACT_ON, 0))
        for result in action_results:
            parameter = result.get_param()
            result_summary = result.get_summary()
            results.append((parameter.get(json_keys.APP_JSON_HASH, '').lower(), result_summary.get(REVERSINGLABS_JSON_POSITIVES, 0),
